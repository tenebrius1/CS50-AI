...
def shortest_path(source, target):
    ...
    """
    # Initialize frontier to just the 
    ...
    while True:
        ...
        # Finds nrighbors for person
        neighbors = neighbors_for_person(node.state)

        for neighbor in neighbors:
            new_node = Node(state=neighbor[1], parent=node, action=neighbor[0])

            # If neighbor is the target, then we have our solution
            ...
            elif not frontier.contains_state(neighbor[1]) and neighbor[1] not in explored:

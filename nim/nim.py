...
class NimAI:
    ...
    def best_future_reward(self, state):
        ...
        for action in actions:
            if (tuple(state), action) in self.q:
                best_q = max(self.q[(tuple(state), action)], best_q)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if not epsilon:
            ...
            for action in actions:
                ...
                elif (tuple(state), action) in self.q:
                    if self.q[(tuple(state), action)] > best_q:
                        best_q = self.q[(tuple(state), action)]
                        best_action = action
        ...
        if epsilon:
            ...
            if best_action != None:
                choices.append(best_action)
                choice = random.choices(choices, weights=[0.1, 0.9]).pop()
                return choice
